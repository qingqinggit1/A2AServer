...
async def generate_with_zhipu_stream(client: AsyncOpenAI, model_name: str, conversation: List[Dict],
...
async def generate_with_zhipu(conversation: List[Dict], model_cfg: Dict,
    ...
    """
    api_key = model_cfg.get("apiKey") or os.getenv("ZHIPU_API_KEY")
    ...
    if stream:
        return generate_with_zhipu_stream(
            client, model_name, conversation, formatted_functions,
            temperature, top_p, max_tokens
        )
    else:
        return await generate_with_zhipu_sync(
            client, model_name, conversation, formatted_functions,
            temperature, top_p, max_tokens
        )
